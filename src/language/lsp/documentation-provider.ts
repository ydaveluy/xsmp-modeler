...
export class XsmpDocumentationProvider extends JSDocDocumentationProvider {
    override getDocumentation(node: AstNode): string | undefined {
        switch (node.$type) {
            case ast.Parameter: return getParameterDescription(node as ast.Parameter);
            case ast.ReturnParameter: return getReturnParameterDescription(node as ast.ReturnParameter);
