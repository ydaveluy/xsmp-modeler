...
export class SmpcatGenerator {
                ...
                case 'None':
                    if (ast.isCollectionLiteral(expression)) {
                        if (ast.isArrayType(type)) {
                            ...
                            return this.convertStructureValue(expression)
                        }
                    }
            }
        }
    ...
    protected convertStructureValue(expression: ast.CollectionLiteral): types.StructureValue {
        //TODO get expression type
        return { '@xsi:type': 'Types:StructureValue', FieldValue: expression.elements.map(e => this.convertValue(undefined, e), this) }
    }
    ...
    protected convertXlink(link: Reference<ast.NamedElement>, context: AstNode): types.Xlink {
        if (link.ref) {
    ...
    protected convertReturnParameter(parameter: ast.ReturnParameter): types.Parameter {
        return {
            ...
            //TODO Description: XsmpUtils.getDescription(parameter),
    ...
    protected convertCatalogue(catalogue: ast.Catalogue): catalogue.Catalogue {
        ...
        return {
            ...
            '@Date': XsmpUtils.getTag(catalogue, 'date')?.content.toString(),
    ...
    public doGenerate(catalogue: ast.Catalogue): string {
    ...
    public generate(catalogue: ast.Catalogue, uri: URI): void {
        ...
        fs.mkdir(outputDir.fsPath, { recursive: true }, (err) => {
            ...
            const dest = UriUtils.joinPath(outputDir, UriUtils.basename(catalogue.$document?.uri as URI).replace(/\.xsmpcat$/, '.smpcat'));

            fs.writeFile(dest.fsPath, this.doGenerate(catalogue), (err) => {
                if (err) {
                    console.error('Error writing file:', err);
                } else {
                    console.log(`File written successfully to ${dest.fsPath}`);
                }
            });
        });
    }
